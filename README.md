...
Runtime config is local and **gitignored** (see [.gitignore](.gitignore): `config/app.toml`, `config/profile.toml`, `config/profile.yaml`, `config/ranking.yaml`, `config/sources.d/*.toml`). Copy templates from `config/examples/` and edit your private copies.

```bash
cp config/examples/app.toml config/app.toml
mkdir -p config/sources.d
cp config/examples/sources.d/rss.toml config/sources.d/my-feed.toml
```
...
- `config/sources.d/*.toml` — enabled sources
...
config/
  examples/        committed templates (`app.toml`, `sources.d/*.toml`, draft YAML examples)
  sources.d/       operator-local source definitions (gitignored except `.gitkeep`)
